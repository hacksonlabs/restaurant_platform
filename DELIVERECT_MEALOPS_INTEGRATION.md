...
- Deliverect menu pushes include `menuType`: delivery+pickup `0`, delivery `1`, pickup `2`, eat-in `3`, and curbside `4`. Phantom maps menuType `0` to pickup+delivery, `1` to delivery, `2` to pickup, `3` to eat-in, and `4` to curbside. Explicit fulfillment config in the provider registration payload outranks the menuType signal. When a menu push carries a menuType signal and the provider location has no explicit fulfillment config, Phantom updates the restaurant's supported fulfillment types and ordering rules to match, so a re-pushed menu can enable delivery on a previously pickup-only location.
- Deliverect Channel order submission uses `orderType`: pickup `1`, delivery `2`, eat-in `3`, curbside `4`, and drive-thru `5`. Phantom maps pickup, delivery, eat-in, and curbside; catering and drive-thru are not mapped. Channel locations without any fulfillment signal default to pickup only.
